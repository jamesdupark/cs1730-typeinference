...
;; =============================================================================
;; Type Inference: test-support.rkt
;; =============================================================================
       ...
       (if (pred actual)
           (lambda () (with-default-check-info*
                          (list (make-check-message
                                 "Predicate was satisfied by actual value")
                                (make-check-actual actual))
                        (lambda () #t)))
           (lambda () (with-default-check-info*
                          (list (make-check-message
                                 "Predicate was not satisfied by actual value")
                                (make-check-actual actual))
                        (lambda () (fail-check)))))))))
...
(define-check (check-with-timeout-exn raw-pred thunk-body)
  (let ([pred
         (cond [(regexp? raw-pred)
                (λ (x) (and (exn:fail? x) (regexp-match raw-pred
                                                        (exn-message x))))]
               [(and (procedure? raw-pred) (procedure-arity-includes?
                                            raw-pred 1))
                raw-pred]
               [else
                (raise-argument-error 'check-exn
...
(define-syntax-rule (*test-equal? name actual expected)
  (test-case name (check-with-timeout-equal?
                   (convert-compile-time-error (thunk actual)) expected)))
...
(define-syntax-rule (*test-pred name pred expr)
  (test-case name (check-with-timeout-pred
                   pred (convert-compile-time-error (thunk expr)))))
...
(define-syntax-rule (*test-exn name pred exn-thunk)
  (test-case name (check-with-timeout-exn
                   pred (convert-compile-time-error exn-thunk))))

; test-raises-error? :: String (Expression ....)
...
(define-syntax-rule (test-raises-error? test-case-name thunk-body)
  (*test-exn test-case-name
             (lambda (exn) #t)
             (thunk (convert-compile-time-error (let () thunk-body)))))
